...
command -v python3 &>/dev/null || err "python3 not found:  apt install python3"
command -v curl   &>/dev/null  || err "curl not found:  apt install curl"
...
if [[ "$PROXY" == socks* ]]; then
  ...
  apt-get install -y -qq python3-socks 2>/dev/null \
    || pip3 install pysocks --break-system-packages -q 2>/dev/null \
...
if ! _pip flask requests; then
  ...
  apt-get update -qq
  apt-get install -y -qq python3-flask python3-requests \
    || err "Failed to install Python packages.  Try: apt install python3-flask python3-requests"
fi

_pip tzdata 2>/dev/null || apt-get install -y -qq python3-tz 2>/dev/null || true
